...
db=orm.SQLiteORM("Control de Stock")
...
def f_nuevo(ventana):
    ...
    db.insertarUno('Productos',date)
    showinfo(title="Nuevo",message="Nuevo producto agregado")
    id=db.mostrar('Productos',where=f'Precio={precio}')[0][0]
    f_limpiar(ventana)
...
def f_actualizar(ventana):
    if not ventana.nombre_texto.get():
        ...
        if mensaje:
            ...
            db.actualizar('Productos', data, f"Nombre='{nombre}'")
